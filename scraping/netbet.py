...
xpath_block = "//div[@class = 'nb-flex-row']"
driver = create_browser()
url = ['https://www.netbet.fr/football/france/ligue-1-uber-eats',
		'https://www.netbet.fr/football/france/ligue-2-bkt',
		'https://www.netbet.fr/football/allemagne/bundesliga',
		'https://www.netbet.fr/football/allemagne/bundesliga-2',
		'https://www.netbet.fr/football/angleterre/premier-league',
		'https://www.netbet.fr/football/angleterre/championship',
		'https://www.netbet.fr/football/espagne/laliga',
		'https://www.netbet.fr/football/espagne/laliga2',
		'https://www.netbet.fr/football/italie/serie-a',
		'https://www.netbet.fr/football/italie/serie-b',
		'https://www.netbet.fr/football/belgique/jupiler-league',
		'https://www.netbet.fr/football/ecosse/premiership',
		'https://www.netbet.fr/football/grece/super-league',
		'https://www.netbet.fr/football/pays-bas/eredivisie',
		'https://www.netbet.fr/football/portugal/primeira-liga',
		'https://www.netbet.fr/football/turquie/super-lig']
mat_netbet = pandas.DataFrame()
k = 0
for urli in url:
	if k in vect_k:
		driver.get(urli)
		...
		for e in element:
			c = cotes(e)
			c.fill_equipe_dom(xpath = ".//div[@class = 'nb-match_actor'][1]",attr = 'innerHTML',regex = '')
			if (c.equipe_dom.replace(' ','').replace('\n','').replace('\t','') in list(mat['dom'])) == False and (c.equipe_dom.replace(' ','').replace('\n','').replace('\t','') in list(mat['ext'])) == False:
				c.fill_equipe_ext(xpath = ".//div[@class = 'nb-match_actor'][2]",attr = 'innerHTML',regex = '')
				c.fill_cote_dom(xpath = ".//a[child::div[@class = 'nb-odds_choice'][text() = '1']]//div[@class='nb-odds_amount']",attr = 'innerHTML',regex = '')
				c.fill_cote_nul(xpath = ".//a[child::div[@class = 'nb-odds_choice'][text() = 'N']]//div[@class='nb-odds_amount']",attr = 'innerHTML',regex = '')
				c.fill_cote_ext(xpath = ".//a[child::div[@class = 'nb-odds_choice'][text() = '2']]//div[@class='nb-odds_amount']",attr = 'innerHTML',regex = '')
		...
		sel = mat['c_ext'] != ''
		mat = mat[sel]
		mat_netbet = mat_netbet.append(mat)
	k += 1
driver.quit()
sel = mat_netbet['dom'] != ''
if sum(sel) > 0:
	mat_netbet = mat_netbet[sel]
mat_netbet.to_csv('C:/Foot_New/Prod/netbet.csv',sep = ';',index = False)
